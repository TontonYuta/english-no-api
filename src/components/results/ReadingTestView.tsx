...
import { LearnedReading } from '../../types';
import {
  BookOpen,
  CheckCircle2,
  XCircle,
...
export const ReadingTestView: React.FC<ReadingTestViewProps> = ({ onReadingsUpdated }) => {
  const [learnedReadings, setLearnedReadings] = useState<LearnedReading[]>(() => getLearnedReadings());
  const [questions, setQuestions] = useState<ReadingTestQuestion[]>([]);
  ...
  const initQuestions = (list: LearnedReading[]) => {
    const generated = generateReadingTestQuestions(list, Math.min(4, Math.max(2, list.length)));
  ...
  useEffect(() => {
    ...
    setLearnedReadings(list);
    initQuestions(list);
  }, []);

  const handleLoadSample = () => {
    SAMPLE_READING_FOR_TEST.forEach((s) => {
      addLearnedReading({
        title: s.title,
        passage: s.passage,
        translationVi: s.translationVi,
        level: s.level,
        topic: 'Office Reading',
        questions: [
          {
            question: s.prompt,
    ...
    const updated = getLearnedReadings();
    setLearnedReadings(updated);
  ...
  const handleToggleMastery = (readingIdOrTitle: string) => {
    toggleReadingMastery(readingIdOrTitle);
    const updated = getLearnedReadings();
    setLearnedReadings(updated);
    onReadingsUpdated?.();
  };

  const handlePlaySnippet = (text: string) => {
  ...
  // Empty state
  if (learnedReadings.length === 0 && questions.length === 0) {
    return (
      <div className="p-8 rounded-none bg-neutral-950 border border-neutral-800 space-y-6 text-center shadow-sm">
        <div className="w-12 h-12 rounded-none bg-purple-950 border border-purple-800 text-purple-400 mx-auto flex items-center justify-center">
          <BookOpen className="w-6 h-6" />
        </div>
        <div className="max-w-md mx-auto space-y-2">
          <h3 className="text-base font-mono font-bold text-white uppercase tracking-tight">
            [ CHƯA CÓ BÀI ĐỌC NÀO TRONG SỔ TAY ]
          </h3>
          <p className="text-xs text-neutral-400 leading-relaxed font-sans">
            Học 1 bài đọc ở tab "Học Mới" và bấm "Đánh dấu đã hiểu", hoặc nạp 3 bài đọc mẫu (Email, Thông báo, Chính sách) để kiểm tra đọc hiểu ngay!
          </p>
        </div>
        ...
        <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
          <button
            ...
            className="px-5 py-2.5 rounded-none bg-purple-600 hover:bg-purple-500 text-white font-mono text-xs font-black transition-all border border-purple-400 flex items-center gap-2 cursor-pointer uppercase tracking-wider"
          >
            <Sparkles className="w-4 h-4 text-amber-300" />
            <span>Nạp 3 bài đọc hiểu mẫu để test ngay</span>
          </button>
        </div>

        {sampleLoadedNotice && (
          <p className="text-xs font-mono text-emerald-400 animate-fade-in">
            ✓ Đã nạp thành công! Đang tạo bài test đọc hiểu...
          </p>
        )}
      </div>
    );
  ...
  if (isTestFinished) {
    ...
    return (
      <div className="p-6 rounded-none bg-neutral-950 border border-neutral-800 space-y-6 shadow-sm border-l-4 border-l-purple-500">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-neutral-800">
          <div>
            ...
            <h3 className="text-xl font-black text-white uppercase tracking-tight mt-1">
              {percentage >= 80 ? 'XUẤT SẮC! KHẢ NĂNG ĐỌC HIỂU CỰC TỐT' : 'HOÀN TẤT! CẦN ÔN LẠI TỪ KHÓA BÀI ĐỌC'}
            </h3>
          </div>

          <button
            type="button"
            onClick={() => initQuestions(learnedReadings)}
            className="px-4 py-2 rounded-none bg-purple-600 hover:bg-purple-500 text-white font-mono text-xs font-black transition-all border border-purple-400 flex items-center gap-2 cursor-pointer uppercase"
          >
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 gap-3">
            {questions.map((q, idx) => {
              ...
              const reading = learnedReadings.find((r) => r.id === q.readingId || r.title === q.title);
              const isMastered = reading?.mastered;
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      type="button"
                      onClick={() => handleToggleMastery(q.readingId || q.title)}
  ...
  return (
    <div className="p-6 rounded-none bg-neutral-950 border border-neutral-800 space-y-6 shadow-sm border-l-4 border-l-purple-500">
      ...
      {/* Passage Snippet Box */}
      <div className="p-4 rounded-none bg-neutral-900/80 border border-neutral-800 space-y-3">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <span className="text-xs font-mono uppercase font-bold text-neutral-400 flex items-center gap-1">
            <FileText className="w-3.5 h-3.5 text-purple-400" />
            <span>ĐOẠN VĂN MỤC TIÊU:</span>
          </span>

          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => handlePlaySnippet(currentQ?.passageSnippet || '')}
              className="px-2 py-1 rounded-none bg-neutral-850 hover:bg-neutral-800 text-sky-400 text-xs font-mono border border-neutral-750 flex items-center gap-1 cursor-pointer"
              title="Nghe audio đoạn văn"
            >
            ...
            <button
              type="button"
              onClick={() => handleToggleMastery(currentQ?.readingId || currentQ?.title)}
        ...
        <p className="text-xs sm:text-sm text-neutral-200 leading-relaxed font-sans italic p-3 bg-neutral-950 border border-neutral-800">
          ...
          >
            <div className="flex items-center justify-between gap-2">
              <span className={`text-xs font-mono font-bold uppercase ${isCurrentCorrect ? 'text-emerald-300' : 'text-rose-300'}`}>
                {isCurrentCorrect ? '[ ĐỌC HIỂU CHÍNH XÁC 🎯 ]' : '[ CHƯA CHUẨN XÁC ]'}
              </span>
              ...
              >
                ...
                <span>Đánh dấu đã hiểu bài đọc này</span>
              </button>
            </div>
      ...
      <div className="flex items-center justify-between pt-2">
        ...
        {currentIndex < questions.length - 1 ? (
          <button
            ...
            className="px-5 py-2.5 rounded-none bg-purple-600 hover:bg-purple-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-mono text-xs font-black transition-all border border-purple-400 flex items-center gap-2 cursor-pointer uppercase"
          >
