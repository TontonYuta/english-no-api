...
import { LearnedGrammar } from '../../types';
import {
  Brain,
  CheckCircle2,
  XCircle,
...
import {
  ...
  GrammarTestQuestion,
  SAMPLE_GRAMMAR_FOR_TEST,
} from '../../utils/quizUtils';

interface GrammarTestViewProps {
  onGrammarUpdated?: () => void;
}

export const GrammarTestView: React.FC<GrammarTestViewProps> = ({ onGrammarUpdated }) => {
  const [learnedGrammar, setLearnedGrammar] = useState<LearnedGrammar[]>(() => getLearnedGrammar());
  const [questions, setQuestions] = useState<GrammarTestQuestion[]>([]);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [selectedAnswers, setSelectedAnswers] = useState<Record<number, number>>({});
  const [isTestFinished, setIsTestFinished] = useState(false);
  const [sampleLoadedNotice, setSampleLoadedNotice] = useState(false);

  const initQuestions = (list: LearnedGrammar[]) => {
    const generated = generateGrammarTestQuestions(list, Math.min(5, Math.max(3, list.length)));
  ...
  useEffect(() => {
    ...
    setLearnedGrammar(list);
    initQuestions(list);
  }, []);

  const handleLoadSample = () => {
    SAMPLE_GRAMMAR_FOR_TEST.forEach((s) => {
      addLearnedGrammar({
        ruleName: s.ruleName,
        formula: s.formula,
    ...
    const updated = getLearnedGrammar();
    setLearnedGrammar(updated);
  ...
  const handleToggleMastery = (ruleIdOrName: string) => {
    toggleGrammarMastery(ruleIdOrName);
    const updated = getLearnedGrammar();
    setLearnedGrammar(updated);
    onGrammarUpdated?.();
  };
  ...
  // Empty state if no grammar learned yet
  if (learnedGrammar.length === 0 && questions.length === 0) {
    return (
      <div className="p-8 rounded-none bg-neutral-950 border border-neutral-800 space-y-6 text-center shadow-sm">
        <div className="w-12 h-12 rounded-none bg-indigo-950 border border-indigo-800 text-indigo-400 mx-auto flex items-center justify-center">
          <Layers className="w-6 h-6" />
        </div>
        <div className="max-w-md mx-auto space-y-2">
          <h3 className="text-base font-mono font-bold text-white uppercase tracking-tight">
            [ CHƯA CÓ CẤU TRÚC NGỮ PHÁP ĐÃ LƯU ]
          </h3>
          <p className="text-xs text-neutral-400 leading-relaxed font-sans">
            Học 1 bài ngữ pháp ở tab "Học Mới" và bấm "Đánh dấu đã hiểu", hoặc nạp 5 mẫu câu TOEIC kinh điển để kiểm tra ngay!
          </p>
        </div>
        ...
        <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
          <button
            ...
            className="px-5 py-2.5 rounded-none bg-indigo-600 hover:bg-indigo-500 text-white font-mono text-xs font-black transition-all border border-indigo-400 flex items-center gap-2 cursor-pointer uppercase tracking-wider"
          >
            <Sparkles className="w-4 h-4 text-amber-300" />
            <span>Nạp 5 mẫu câu TOEIC A1–B2 để test ngay</span>
          </button>
        </div>

        {sampleLoadedNotice && (
          <p className="text-xs font-mono text-emerald-400 animate-fade-in">
            ✓ Đã nạp thành công! Đang tạo bài test ngữ pháp...
          </p>
        )}
      </div>
    );
  ...
  if (isTestFinished) {
    ...
    return (
      <div className="p-6 rounded-none bg-neutral-950 border border-neutral-800 space-y-6 shadow-sm border-l-4 border-l-indigo-500">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-neutral-800">
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-mono font-bold px-2 py-0.5 rounded-none bg-indigo-950 text-indigo-300 border border-indigo-800">
                [ KẾT QUẢ TEST NGỮ PHÁP ]
              </span>
            ...
            <h3 className="text-xl font-black text-white uppercase tracking-tight mt-1">
              {percentage >= 80 ? 'XUẤT SẮC! BẠN ĐÃ NẮM VỮNG CẤU TRÚC' : 'HOÀN TẤT! CẦN ÔN THÊM CÔNG THỨC'}
            </h3>
          </div>

          <button
            type="button"
            onClick={() => initQuestions(learnedGrammar)}
            className="px-4 py-2 rounded-none bg-indigo-600 hover:bg-indigo-500 text-white font-mono text-xs font-black transition-all border border-indigo-400 flex items-center gap-2 cursor-pointer uppercase"
          >
        ...
        <div className="space-y-3">
          <h4 className="text-xs font-mono font-bold text-neutral-300 uppercase">
            CHI TIẾT TỪNG CÂU VÀ TRẠNG THÁI GHI NHỚ:
          </h4>
          <div className="grid grid-cols-1 gap-3">
            {questions.map((q, idx) => {
              const selectedOpt = selectedAnswers[idx];
              const isCorrect = selectedOpt === q.correctIndex;
              const rule = learnedGrammar.find((g) => g.id === q.ruleId || g.ruleName === q.ruleName);
              const isMastered = rule?.mastered;
                ...
                >
                  <div className="space-y-1 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="text-xs font-mono text-amber-300 bg-neutral-900 px-1.5 py-0.5 border border-neutral-800">
                        {q.formula}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      type="button"
                      onClick={() => handleToggleMastery(q.ruleId || q.ruleName)}
                      ...
                      title={isMastered ? 'Đã hiểu vững' : 'Bấm để đánh dấu đã hiểu'}
                    >
    ...
    <div className="p-6 rounded-none bg-neutral-950 border border-neutral-800 space-y-6 shadow-sm border-l-4 border-l-indigo-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-neutral-800">
        <div>
          <div className="flex items-center gap-2">
            <span className="text-xs font-mono font-bold px-2 py-0.5 rounded-none bg-indigo-950 text-indigo-300 border border-indigo-800">
              [ KIỂM TRA NGỮ PHÁP ĐÃ HỌC ]
            </span>
            <span className="text-xs font-mono text-neutral-400">
              CÂU {currentIndex + 1} / {questions.length}
            </span>
          </div>
          <h3 className="text-base sm:text-lg font-black text-white tracking-tight uppercase mt-1 flex items-center gap-2">
            <span>{currentQ?.ruleName}</span>
          </h3>
        </div>
      ...
      <div className="p-5 rounded-none bg-neutral-900/60 border border-neutral-800 space-y-4">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <span className="text-xs font-mono text-indigo-300 bg-indigo-950/80 px-2 py-0.5 rounded-none border border-indigo-800 inline-block font-bold">
            CÔNG THỨC: {currentQ?.formula}
          </span>

          <button
            type="button"
            onClick={() => handleToggleMastery(currentQ?.ruleId || currentQ?.ruleName)}
          ...
          >
            ...
            <span>{currentQ?.mastered ? 'ĐÃ HIỂU ⭐' : 'ĐÁNH DẤU ĐÃ HIỂU'}</span>
          </button>
        </div>
          ...
          >
            <div className="flex items-center justify-between gap-2">
              ...
              >
                ...
                <span>Đánh dấu đã hiểu cấu trúc này</span>
              </button>
            </div>
      ...
      <div className="flex items-center justify-between pt-2">
        ...
        {currentIndex < questions.length - 1 ? (
          <button
            ...
            className="px-5 py-2.5 rounded-none bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-mono text-xs font-black transition-all border border-indigo-400 flex items-center gap-2 cursor-pointer uppercase"
          >
